...
var defaultagent=request.agent(host,{ca:cert});
var loginparams={email:"admin@example.com",key:"key"};

function getTestAgent(login) {
    var agent=request.agent(host,{ca:cert});
    if (login) {
        it("try to login",function (done) {
            agent.get("/api/kansa/login")
                .query(loginparams)
...
describe("Member list",function () {
    it ("Returns 'success' as status and test data member list.", function (done) {
        defaultagent.get("/api/kansa/public/people")
...
describe("Country statistics", function () {
    it("Returns country statistics", function (done) {
        defaultagent.get("/api/kansa/public/stats").expect(200,{status:'success', members: memberstats}).end(done)
    })
})

describe("Login",function () {
    context("Successful login",function () {
        var testagent=getTestAgent(true);
...
describe("Logout",function() {
    ...
    context("Not logged in", function () {
        it("logout should be unauthorized", function (done) {
            defaultagent.get("/api/kansa/logout")
                .expect(401,{status:"unauthorized"})
                .end(done);
        });
        it ("gets unauthorized from /api/kansa/user",function (done) {
